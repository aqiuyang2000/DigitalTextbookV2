# outline_editor_widget.py
# 功能：将提纲编辑器从一个独立的对话框（QDialog）重构为一个可嵌入的部件（QWidget）。
...
class OutlineEditorWidget(QWidget): # *** 核心修改 1：继承自 QWidget ***
    """
    一个可嵌入的部件，用于编辑树状结构的提纲。
    """
    ...
    def __init__(self, parent=None):
        ...
        # *** 核心修改 2：移除了所有 QDialog 特有的代码 (setWindowTitle, setGeometry) ***
        self.setMinimumWidth(250) # 设置一个合适的最小宽度
        ...
        def _recursive_add(parent_item, nodes):
            for node in nodes:
                ...
                if node.get('children'):
                    _recursive_add(title_item, node['children'])
    ...
    def get_tree_data(self) -> list:
        ...
        def _recursive_get(parent_item, data_list):
            for row in range(parent_item.rowCount()):
                ...
                node = {'title': title_item.text(), 'page': int(page_item.text()) if page_item.text().isdigit() else 1, 'children': []}
                data_list.append(node)
                if title_item.hasChildren():
                    _recursive_get(title_item, node['children'])
    ...
    def _on_item_clicked(self, index: QModelIndex):
        ...
        if page_item and page_item.text().isdigit():
            self.item_clicked.emit(int(page_item.text()))
    ...
    def _add_item(self, parent_item, row=-1):
        title, ok = QInputDialog.getText(self, "添加提纲项", "请输入标题:")
        if ok and title:
            title_item = QStandardItem(title)
            page_item = QStandardItem(str(self._current_page_for_new_items))
            if row == -1: parent_item.appendRow([title_item, page_item])
            else: parent_item.insertRow(row, [title_item, page_item])
    ...
    def _open_context_menu(self, position):
        ...
        add_child_action.setEnabled(is_single_selection); insert_above_action.setEnabled(is_single_selection)
        insert_below_action.setEnabled(is_single_selection); rename_action.setEnabled(is_single_selection)
        ...
        if action == add_child_action: self._add_item(parent_item=item)
        elif action == insert_above_action: self._add_item(parent_item=(item.parent() or self.model.invisibleRootItem()), row=index.row())
        elif action == insert_below_action: self._add_item(parent_item=(item.parent() or self.model.invisibleRootItem()), row=index.row() + 1)
        elif action == rename_action: self._on_item_double_clicked(index.siblingAtColumn(0))
        elif action == delete_action: self._delete_items()

    def _import_from_excel(self):
        path, _ = QFileDialog.getOpenFileName(self, "从 Excel 导入提纲", "", "Excel 文件 (*.xlsx)")
        if not path: return
        reply = QMessageBox.question(self, "确认导入", "导入将覆盖当前所有提纲，确定要继续吗？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            try:
                data = ExcelProcessor.import_from_excel(path); self.populate_tree(data); self._on_model_changed()
    ...
    def _export_to_excel(self):
        ...
        if not path: return
        try:
            data = self.get_tree_data(); ExcelProcessor.export_to_excel(data, path)
